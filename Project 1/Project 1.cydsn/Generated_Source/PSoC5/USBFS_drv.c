...
CY_ISR(USBFS_EP_0_ISR)
{
    ...
    if (bRegTemp & USBFS_MODE_ACKD)
    {
        ...
        if (bRegTemp & USBFS_MODE_SETUP_RCVD)
        {
            ...
            {
                USBFS_HandleSetup();
                if(USBFS_ep0Mode & USBFS_MODE_SETUP_RCVD)
                {
        ...
        if(modifyReg != 0u)
        {
            ...
            if((bRegTemp & USBFS_MODE_SETUP_RCVD) == 0)  /* Check if SETUP bit is not set, otherwise exit */
            {
                /* Update the count register */
                bRegTemp = USBFS_ep0Toggle | USBFS_ep0Count;
                CY_SET_REG8(USBFS_EP0_CNT_PTR, bRegTemp);
                if(bRegTemp == CY_GET_REG8(USBFS_EP0_CNT_PTR))   /* continue if writing was succesful */
                {
                    do
                    {
...
void USBFS_HandleSetup(void) 
{
    ...
    if(requestHandled & USBFS_MODE_SETUP_RCVD)
    {
...
*     current packet.
*  USBFS_lastPacketSize - remebers the USBFS_ep0Count value for the
*     next packet.
*  USBFS_transferByteCount - suma of the previos bytes transferred
...
void USBFS_LoadEP0(void) 
{
    ...
    USBFS_ep0Count = 0u;
    while ((USBFS_currentTD.count > 0u) && (USBFS_ep0Count < 8u))
    {
        USBFS_EP0_DR0_PTR[USBFS_ep0Count] = *USBFS_currentTD.pData++;
        USBFS_ep0Count++;
...
void USBFS_ControlWriteDataStage(void) 
{
    ...
    while ((USBFS_currentTD.count > 0u) && (USBFS_ep0Count > 0u))
    {
        *USBFS_currentTD.pData++ = CY_GET_REG8(pReg++);
        USBFS_ep0Count--;
        USBFS_currentTD.count--;
    }
    /* Update the data toggle */
