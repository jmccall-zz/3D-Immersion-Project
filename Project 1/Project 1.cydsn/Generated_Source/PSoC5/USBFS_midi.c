...
const uint8 CYCODE USBFS_MIDI_SIZE[] = {
...
volatile uint8 USBFS_midiInBuffer[USBFS_MIDI_IN_BUFF_SIZE];   /* Input endpoint buffer */
volatile uint8 USBFS_midiOutBuffer[USBFS_MIDI_OUT_BUFF_SIZE]; /* Output endpoint buffer */
...
#if (USBFS_MIDI_EXT_MODE >= USBFS_ONE_EXT_INTRF)
    volatile USBFS_MIDI_RX_STATUS USBFS_MIDI1_Event;          /* MIDI RX status structure */
    volatile uint8 USBFS_MIDI1_TxRunStat;                                /* MIDI Output running status */
    volatile uint8 USBFS_MIDI1_InqFlags;                                 /* Device inquiry flag */    
    
#if (USBFS_MIDI_EXT_MODE >= USBFS_TWO_EXT_INTRF)
    volatile USBFS_MIDI_RX_STATUS USBFS_MIDI2_Event;          /* MIDI RX status structure */
    volatile uint8 USBFS_MIDI2_TxRunStat;                                /* MIDI Output running status */
...
void USBFS_PrepareInBuffer(uint8 ic, uint8 *srcBuff, uint8 eventLen, uint8 cable)
                                                                 ;
#if (USBFS_MIDI_EXT_MODE >= USBFS_ONE_EXT_INTRF)
    void USBFS_MIDI_Init(void) ;

    uint8 USBFS_MIDI1_GetEvent(void) ;
    void USBFS_MIDI1_ProcessUsbOut(uint8 *epBuf) ;
    
    #if (USBFS_MIDI_EXT_MODE >= USBFS_TWO_EXT_INTRF)
        uint8 USBFS_MIDI2_GetEvent(void) ;
        void USBFS_MIDI2_ProcessUsbOut(uint8 *epBuf) 
                                                    ;
    #endif /* End USBFS_MIDI_EXT_MODE >= USBFS_TWO_EXT_INTRF */
#endif /* End USBFS_MIDI_EXT_MODE >= USBFS_ONE_EXT_INTRF */
...
void USBFS_MIDI_EP_Init(void) 
{
    USBFS_midiInPointer = 0u;
        ...
        USBFS_LoadInEP(USBFS_midi_in_ep, (uint8 *)USBFS_midiInBuffer,
                                                                                USBFS_MIDI_IN_BUFF_SIZE);
        /* Init DMA configurations for OUT EP*/
        USBFS_ReadOutEP(USBFS_midi_out_ep, (uint8 *)USBFS_midiOutBuffer, 
    ...
    if (USBFS_GetEPState(USBFS_midi_out_ep) == USBFS_OUT_BUFFER_FULL)
    {
        outLength = USBFS_GetEPCount(USBFS_midi_out_ep);
            ...
            outLength = USBFS_ReadOutEP(USBFS_midi_out_ep, 
                                                   (uint8 *)USBFS_midiOutBuffer, outLength);
            #if(USBFS_EP_MM == USBFS__EP_DMAMANUAL)
                do  /* wait for DMA transfer complete */
                {
                    CyDmaChStatus(USBFS_DmaChan[USBFS_midi_out_ep], NULL, &dmaState);
            ...
            while (outPointer < outLength)
            {
                ...
                /* Route USB MIDI to the External connection */
                    ...
                    if ((USBFS_midiOutBuffer[outPointer] & USBFS_CABLE_MASK) == 
                        USBFS_MIDI_CABLE_00)
                    {
                        USBFS_MIDI1_ProcessUsbOut((uint8 *)&USBFS_midiOutBuffer[outPointer]);
                    }
                    else if ((USBFS_midiOutBuffer[outPointer] & USBFS_CABLE_MASK) == 
                             USBFS_MIDI_CABLE_01)
                    {
                        #if (USBFS_MIDI_EXT_MODE >= USBFS_TWO_EXT_INTRF)
                            USBFS_MIDI2_ProcessUsbOut((uint8 *)&USBFS_midiOutBuffer[outPointer]);
                        #endif /* End USBFS_MIDI_EXT_MODE >= USBFS_TWO_EXT_INTRF */
                    }
                    else
                    {
                ...
                USBFS_callbackLocalMidiEvent(
                    ...
                    (uint8 *)&USBFS_midiOutBuffer[outPointer + USBFS_EVENT_BYTE1]);
...
void USBFS_MIDI_IN_EP_Service(void) 
{
    ...
    if ((USBFS_midiInPointer != 0u) && 
        (USBFS_GetEPState(USBFS_midi_in_ep) == USBFS_EVENT_PENDING))
    {
        #if(USBFS_EP_MM != USBFS__EP_DMAAUTO)
            USBFS_LoadInEP(USBFS_midi_in_ep, (uint8 *)USBFS_midiInBuffer,
                                                                                USBFS_midiInPointer);
            ...
            /* rearm IN EP */
            USBFS_LoadInEP(USBFS_midi_in_ep, NULL, USBFS_midiInPointer);
        #endif /* End USBFS_EP_MM != USBFS__EP_DMAAUTO*/
    ...
    */
        ...
        do 
        {
            ...
            if (USBFS_midiInPointer <= (USBFS_MIDI_IN_BUFF_SIZE - USBFS_EVENT_LENGTH))
            {
             ...
             (USBFS_midiInPointer <= (USBFS_MIDI_IN_BUFF_SIZE - USBFS_EVENT_LENGTH)));
    #endif /* End USBFS_MIDI_EXT_MODE >= USBFS_ONE_EXT_INTRF */
                                                            ...
                                                            
{
                ...
                (USBFS_EP[USBFS_midi_in_ep].bufferSize - USBFS_EVENT_LENGTH))
    {
        ...
        {   /* only SysEx message is greater than 4 bytes */
            do
            {
                USBFS_PrepareInBuffer(USBFS_MIDI_SYSEX, midiMsg, 
                                                 USBFS_EVENT_BYTE3, cable);
                ic -= USBFS_EVENT_BYTE3;
                midiMsg += USBFS_EVENT_BYTE3;
                if (USBFS_midiInPointer > 
                    (USBFS_EP[USBFS_midi_in_ep].bufferSize - USBFS_EVENT_LENGTH))
                {
                    USBFS_MIDI_IN_EP_Service();
    ...
    else if((ic < USBFS_EVENT_LENGTH) || (ic == USBFS_MIDI_SYSEX))
    {
        ...
        else if (srcBuffZero < USBFS_MIDI_SYSEX)
        {
            USBFS_midiInBuffer[USBFS_midiInPointer] =
                                                    ((srcBuffZero >> 4u) & USBFS_CIN_MASK) | cable;
            USBFS_midiInPointer++;
        }
                                                                ...
                                                                
    {
        ...
        */
        if ((mData & USBFS_MIDI_STATUS_BYTE_MASK) != 0u)
        {
            ...
            {
                if ((mData & USBFS_MIDI_SINGLE_BYTE_MASK) != 0u) /* System Real-Time Messages(singl byte) */
                {
                    midiReturn = mData;
                }
