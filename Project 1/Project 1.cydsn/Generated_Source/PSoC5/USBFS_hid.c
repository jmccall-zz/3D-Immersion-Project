...
void USBFS_FindHidClassDecriptor() 
{
    ...
    USBFS_currentTD.count = pDescr[0u];
...
void USBFS_FindReportDescriptor() 
{
    ...
    pTmp += 3;
    pDescr = (volatile uint8 *)pTmp->p_list;
    USBFS_currentTD.count =  (((uint16)pDescr[1] << 8u) | pDescr[0u]);
    ...
    if(interfaceN < USBFS_MAX_INTERFACES_NUMBER)
    {
           ...
           (reportType <= USBFS_HID_GET_REPORT_FEATURE))
        {
            ...
            if(pTmp->c >= reportType)
            {
                ...
                USBFS_currentTD.pData = pTD->pData;   /* Bufer pointer */
                USBFS_currentTD.count = pTD->count;   /* Bufer Size */
