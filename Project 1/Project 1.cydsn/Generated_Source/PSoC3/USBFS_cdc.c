...
*
*******************************************************************************/
uint8 USBFS_DispatchCDCClassRqst() 
{
...
***************************************/
    ...
    void USBFS_PutString(char8* string) 
    {
        ...
        do
        {
            ...
            send_length = (str_length > USBFS_EP[USBFS_cdc_data_in_ep].bufferSize) ?
             ...
             /* Enable IN transfer */
            USBFS_LoadInEP(USBFS_cdc_data_in_ep, (uint8 *)string, send_length);
    ...
    *******************************************************************************/
    ...
    {
        USBFS_LoadInEP(USBFS_cdc_data_in_ep, (uint8 *)&txDataByte, 1u);
    }
    ...
    *******************************************************************************/
    void USBFS_PutCRLF(void) 
    {
        ...
        USBFS_LoadInEP(USBFS_cdc_data_in_ep, (uint8 *)txData, 2u);
    }
    ...
    *******************************************************************************/
    uint8 USBFS_GetChar(void) 
    {
        ...
        USBFS_ReadOutEP(USBFS_cdc_data_out_ep, &rxData, 1u);
    ...
    *******************************************************************************/
    uint32 USBFS_GetDTERate(void) 
    {
        ...
        /* Data terminal rate has little endian format. */
            ...
            ((uint8 *)&rate)[0u] = USBFS_lineCoding[USBFS_LINE_CODING_RATE + 3u];
            ((uint8 *)&rate)[1u] = USBFS_lineCoding[USBFS_LINE_CODING_RATE + 2u];
            ((uint8 *)&rate)[2u] = USBFS_lineCoding[USBFS_LINE_CODING_RATE + 1u];
            ((uint8 *)&rate)[3u] = USBFS_lineCoding[USBFS_LINE_CODING_RATE];
        #elif defined(__GNUC__) || defined(__ARMCC_VERSION)
            /* ARM compilers (GCC and RealView) are little-endian */
            USBFS_CONVERT_DWORD *convert =
                (USBFS_CONVERT_DWORD *)&USBFS_lineCoding[USBFS_LINE_CODING_RATE];
            rate = convert->dword;
