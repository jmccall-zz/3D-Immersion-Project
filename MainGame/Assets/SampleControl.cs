...
public class SampleControl : MonoBehaviour {
	...
	public bool record_joint_positions = false;
	public bool record_joint_rotations = false;
	...
	void Update () {
		...
		if (time_diff >= capture_rate) {
			...
			if (zig_control.usersInView == 1) {
				sampler.SampleAllJoints(skeleton, user_id, db_control.cylinder_reach_scene, record_rotations : record_joint_rotations, record_positions : record_joint_positions);
			}
		}
	}
}
